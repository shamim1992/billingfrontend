...
const DoctorCollection = () => {
  ...
  // Helper function to get investigation total for a bill
  const getInvestigationTotal = (bill) => {
    const investigationItems = bill.billingItems?.filter(
      item => item.category?.toLowerCase() === 'investigation'
    ) || [];
    return investigationItems.reduce((sum, item) => sum + (item.total || 0), 0);
  };
  ...
  const calculateSummary = (bills) => {
    const newSummary = bills.reduce((acc, bill) => {
      ...
      // Calculate investigation fees
      const investigationTotal = getInvestigationTotal(bill);
      ...
      // Update total amount and investigation fees
      acc.totalAmount += amount;
      acc.investigationFees += investigationTotal;
  ...
  const handleExport = () => {
    ...
    const htmlContent = `
      ...
      <html>
        <head>
          <title>Doctor Collection Report - ${dayjs().format('YYYY-MM-DD')}</title>
          <style>
            body { 
        ...
        <body>
          <div class="header">
            <div class="report-title">Investigation Collection Report</div>
          ...
          <div class="summary-card">
            ...
            <table>
              ...
              <tr>
                ...
                <td>${formatCurrency(summary.investigationFees)}</td>
              ...
              ${filteredBills.map(bill => {
                ...
                return `
                  <tr>
                    ...
                    <td>${formatCurrency(investigationTotal)}</td>
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: row => getInvestigationTotal(row),
      format: row => formatCurrency(getInvestigationTotal(row)),
      sortable: true,
    },
    {
      name: 'Amount',
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
      ...
      <div className="col-span-2 grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-yellow-50 p-4 rounded-lg">
          <div className="text-xs font-semibold text-yellow-700">Investigation Fees</div>
          <div className="text-sm font-bold text-yellow-900">{formatCurrency(summary.investigationFees)}</div>
        </div>
                ...
                <table className="min-w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50">
                      ...
                      <th className="px-4 py-2 border">Investigation Fees</th>
                  ...
                  <tbody>
                    <tr className="text-center">
                      ...
                      <td className="px-4 py-2 border text-yellow-600">
                        {formatCurrency(summary.investigationFees)}
                      </td>
